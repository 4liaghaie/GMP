...
const steps = [
  {
    icon: Box,
    title: "ثبت اطلاعات کالا یا بار",
    description:
      "صاحب کالا مشخصات بار و کالاهای مربوط را ثبت می‌کند تا پس از بررسی در بازار نمایش داده شود.",
  },
  {
    icon: FileCheck2,
  ...
  },
  {
    ...
    description:
      "کاربران می‌توانند ثبت سفارش‌ها و بارهای تاییدشده را جست‌وجو و جزئیات مرتبط را مقایسه کنند.",
  },
  {
    icon: Handshake,
...
const paths = [
  {
    title: "اگر صاحب کالا یا بار هستید",
    description:
      "اطلاعات بار و کالاها را ثبت کنید تا پس از تایید مدیر در مارکت‌پلیس نمایش داده شود.",
    items: [
      "ثبت مشخصات کلی بار",
      "افزودن اطلاعات حمل و مقصد",
      "ثبت کالاها و فایل مربوط",
    ],
    href: "/add-need",
    action: "ثبت بار",
  ...
  },
  {
    ...
    description:
      "اطلاعات ثبت سفارش و کالاها را وارد کنید تا فرصت‌های مرتبط در بازار قابل پیدا کردن باشند.",
    items: [
      "ثبت اطلاعات عمومی سفارش",
      "تکمیل اطلاعات مالی موردنیاز",
      "افزودن کالاها و فایل سفارش",
    ],
    href: "/add-order",
    action: "ثبت سفارش",
  ...
  return (
    <div dir="rtl" className="space-y-8 text-right">
      <section className="relative isolate overflow-hidden rounded-[2rem] bg-[#072f53] px-6 py-12 text-center text-white shadow-xl sm:px-10 sm:py-16">
        ...
        <span className="inline-flex rounded-full border border-white/15 bg-white/10 px-4 py-2 text-xs font-bold text-[#68dce2]">
          راهنمای استفاده از GMP
        </span>
        <h1 className="mx-auto mt-5 max-w-3xl text-3xl font-black leading-[1.6] sm:text-4xl">
          از ثبت اطلاعات تا پیدا کردن شریک تجاری
        </h1>
        <p className="mx-auto mt-4 max-w-2xl text-sm leading-8 text-white/70 sm:text-base">
          مسیر استفاده از GMP ساده است: اطلاعات را ثبت کنید، منتظر بررسی بمانید،
          فرصت‌های تاییدشده را پیدا کنید و برای همکاری ارتباط بگیرید.
        </p>
      </section>
      ...
      <section className="py-4">
        ...
        <div className="relative mt-12 grid gap-5 md:grid-cols-4">
          ...
          {steps.map(({ icon: Icon, title, description }, index) => (
            <article
              key={title}
              className="relative rounded-[1.5rem] border bg-card px-5 pb-6 pt-10 text-center shadow-sm"
            >
              <span className="absolute -top-4 right-1/2 grid h-9 w-9 translate-x-1/2 place-items-center rounded-full bg-[#078e9d] text-sm font-black text-white ring-8 ring-background">
                {index + 1}
              </span>
              <Icon
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        {paths.map((path) => (
          ...
          >
            ...
            <Button
              asChild
              className={`mt-6 h-11 rounded-xl px-5 text-white ${path.accent}`}
            >
              <Link href={path.href}>
                {path.action}
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
          ...
          <div>
            ...
            <p className="mt-2 text-sm leading-7 text-muted-foreground">
              ثبت سفارش‌ها و بارها پیش از نمایش عمومی توسط مدیر بررسی می‌شوند.
              نتیجه تایید یا رد از طریق اعلان‌های حساب کاربری به ثبت‌کننده اطلاع
              داده می‌شود.
            </p>
          </div>
        </div>
      ...
      <section className="flex flex-col gap-5 rounded-[1.75rem] border bg-card p-7 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm leading-7 text-muted-foreground">
            پس از تایید حساب کاربری، همه بخش‌های بازار در دسترس شما قرار
            می‌گیرد.
          </p>
        </div>
        <div className="flex gap-3">
          ...
          <Button
            asChild
            className="h-11 rounded-xl bg-[#078e9d] hover:bg-[#087d89]"
          >
